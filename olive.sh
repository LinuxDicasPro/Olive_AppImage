...
APP=olive-git
BIN="olive-editor"
DEPENDENCES="ffmpeg4.4 openimageio opentimelineio portaudio qt5-x11extras qt5-svg jack2 libglvnd"
...
function _create_AppRun() {
	...
	prg=org.olivevideoeditor.Olive
...
function _extract_main_package() {
	mkdir -p base
	rm -R -f ./base/*
	tar fx "$(find ./"$APP".AppDir -name "$APP-r[0-9]*zst" | head -1)" -C ./base/
...
mv ./Olive-x86_64.AppImage ./Olive-$VERSION-x86_64.AppImage
